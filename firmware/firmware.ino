      ...
      //  scan all                                            -> str: list of dicts for info on each device
      {
        ...
        case 'b':
          ...
          if (bufferIndex >= 9) {
            ...
            uint32_t currentBaudrate = ((uint32_t)buffer[3] << 24 | (uint32_t)buffer[4] << 16 | (uint32_t)buffer[5] << 8 | (uint32_t)buffer[6]);
            uint32_t newBaudrate = ((uint32_t)buffer[7] << 24 | (uint32_t)buffer[8] << 16 | (uint32_t)buffer[9] << 8 | (uint32_t)buffer[10]);
...
void writeStageID() {
  EEPROM.update(STAGE_ID_ADDR, STAGE_ID);
}


void readStageID() {
  uint8_t id = EEPROM.read(STAGE_ID_ADDR);

  if (id == 0xFF) {
    cmd.print("{\"stage_id\": -1}");
  } else {
    cmd.print("{\"stage_id\":");
    cmd.print(String(id));
    cmd.println("}");
