...
class SerialConnection:
    ...
    def __repr__(self) -> str:
        return f"SerialConnection(serial_port={self.serial_port}, baudrate={self.baudrate}, timeout={self.timeout})"

    def __str__(self) -> str:
        return f"SerialConnection: {self.serial_port} @ {self.baudrate} baud, timeout={self.timeout}"
    ...
    def connect(self) -> "SerialConnection":
        if not self.connected:
            ...
            if self.connection.is_open:
                ...
                logging.error(
                    f"Failed to open serial port {self.serial_port}."
                )
    ...
    def _encode(self, data: Any, order: str = None) -> bytes:
    ...
    def send(self, command: str, data: Any = None, order: str = None) -> None:
        ...
        assert isinstance(data, (list, tuple, type(None)))
        assert isinstance(order, (str, type(None)))
    ...
    def read_bytes(
        ...
        if len(raw_data) != n_bytes:
            raise ValueError(
                f"Did not receive {n_bytes} bytes from serial port"
            )
...
class StageAPI(SerialConnection):
    def __init__(
        self, serial_port: str, baudrate: int = 115200, timeout: int = 1
    ) -> None:
    ...
    def set_position_multiple(
        self, position_tuples: list[tuple[int, int]]
    ) -> None:
        """
    ...
    def set_device_id(
        self, current_device_id: int, new_device_id: int
    ) -> None:
        """
    ...
    def _resolve_operating_mode_code(self, op_mode: str) -> int:
        ...
        """
        return OP_MODE_LOOKUP.get(op_mode, None)
...
class StageAxis(StageAPI):
    ...
    def set_velocity(self, velocity: int) -> None:
        ...
        """
        assert (
            velocity >= 0 and velocity <= self.velocity_max
        ), f"Invalid velocity: {velocity}"

        self.controller.set_velocity(device_id=self.id, velocity=velocity)
        logging.debug(f"Set velocity: {velocity}")

    def set_operating_mode(self, mode: str | int) -> None:
        """
        Set the operating mode of the device.
        """
        assert mode is not None, f"Invalid operating mode: {mode}"

        if isinstance(mode, str):
            mode = self._resolve_operating_mode_code(op_mode=mode)

        self.controller.set_operating_mode(device_id=self.id, op_mode=mode)
        logging.debug(f"Set operating mode: {mode}")
